"""
PDDChannel 精简实现（编排委托 ws_account / ws_lifecycle / ws_inbound_pipeline）。

.. deprecated::
    新代码请 ``from Channel.pinduoduo.pdd_channel import PDDChannel``；
    本文件名 ``pdd_chnnel`` 仅为历史拼写兼容保留。
"""
...
class PDDChannel(Channel):
    ...
    查询与全局状态请使用 ``Channel.pinduoduo.pdd_channel`` / ``core.channel_facade``。
    """
...
def _warn_if_legacy_direct_import() -> None:
    import inspect
    import warnings

    for frame in inspect.stack()[1:]:
        mod = frame.frame.f_globals.get("__name__", "")
        if mod in ("Channel.pinduoduo.pdd_channel", "__main__"):
            return
        if mod.startswith("test."):
            return
    warnings.warn(
        "请从 Channel.pinduoduo.pdd_channel 导入 PDDChannel，pdd_chnnel 仅为历史拼写兼容。",
        DeprecationWarning,
        stacklevel=2,
    )


_warn_if_legacy_direct_import()
