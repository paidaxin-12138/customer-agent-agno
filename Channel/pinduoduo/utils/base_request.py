...
# 与登录浏览器一致的 UA 随 Cookie 一并持久化（非真实 Cookie，请求前会剥离）
CLIENT_UA_EMBED_KEY = "___client_ua___"
...
class BaseRequest:
    ...
    def _apply_cookie_dict(self, raw: Dict[str, Any]) -> None:
        """从解析后的 Cookie 字典剥离内嵌 UA 并写入 default_headers（不随 requests.cookies 发送）。"""
        data = dict(raw)
        ua = data.pop(CLIENT_UA_EMBED_KEY, None)
        if isinstance(ua, str) and ua.strip():
            self.default_headers["User-Agent"] = ua.strip()
        self.cookies = data
        ...
        try:
            ...
            if account_info:
                ...
                if isinstance(cookies_data, str):
                    try:
                        self._apply_cookie_dict(json.loads(cookies_data))
                    except json.JSONDecodeError:
                        self.logger.error(f"解析账号 {self.account_name} 的cookies失败")
                        self.cookies = {}
                elif isinstance(cookies_data, dict):
                    self._apply_cookie_dict(cookies_data)
                else:
    ...
    def update_cookies(self, new_cookies: Union[Dict, str]) -> None:
        ...
        if isinstance(new_cookies, str):
            try:
                self._apply_cookie_dict(json.loads(new_cookies))
            except json.JSONDecodeError:
                self.logger.error("更新cookies失败: JSON解析错误")
        elif isinstance(new_cookies, dict):
            self._apply_cookie_dict(new_cookies)
        else:
