        ...
        try:
            ...
            result = loop.run_until_complete(
                login_pdd(
                    ...
                    login_type=self.account_data.get("login_type", "password"),
                )
            )
...
class AccountCard(CardWidget):
    ...
    def createInfoWidget(self):
        ...
        shop_name_label.setStyleSheet("color: #F2F2F7;")
    ...
    def createInfoRow(self, label_text: str, value_text: str):
        ...
        label.setStyleSheet("color: #9EA6B8; font-weight: 500;")
        label.setFixedWidth(60)
        
        # 值
        value = BodyLabel(value_text)
        value.setStyleSheet("color: #F2F2F7;")
...
class UserManagerWidget(QFrame):
    ...
    def onAddAccountLoginFinished(self, result: object):
        ...
        if not result:
            QMessageBox.warning(self, "添加失败", "登录验证失败，请重试（二维码登录请确认已完成扫码）。")
            return
...
class EditAccountDialog(QDialog):
    ...
    def setupUI(self):
        ...
        self.channel_label.setStyleSheet("color: #9EA6B8; font-weight: bold;")
        ...
        self.shop_label.setStyleSheet("color: #9EA6B8; font-weight: bold;")
...
class AddAccountDialog(QDialog):
    ...
    def setupUI(self):
        ...
        self.login_type_combo = QComboBox(self)
        self.login_type_combo.addItem("账号密码登录", "password")
        self.login_type_combo.addItem("二维码登录", "qrcode")
        self.login_type_combo.setFixedSize(300, 32)
        
        # 账号信息
        self.username_edit = LineEdit(self)
        self.username_edit.setPlaceholderText("请输入拼多多账号用户名（二维码登录可选）")
        ...
        layout.addRow("登录方式:", self.login_type_combo)
        layout.addRow("用户名:", self.username_edit)
        layout.addRow("密码:", self.password_widget)
    ...
    def _on_login_type_changed(self):
        login_type = self.login_type_combo.currentData()
        is_password_mode = login_type == "password"
        self.password_widget.setVisible(is_password_mode)
        self.password_edit.setEnabled(is_password_mode)
        if is_password_mode:
            self.username_edit.setPlaceholderText("请输入拼多多账号用户名")
        else:
            self.username_edit.setPlaceholderText("可选：填写账号标识（不填则自动生成）")
    ...
    def getAccountInfo(self) -> dict:
        ...
        login_type = self.login_type_combo.currentData()
        return {
            "username": self.username_edit.text().strip() or "pdd_qrcode_login",
            "password": self.password_edit.text().strip() if login_type == "password" else "",
